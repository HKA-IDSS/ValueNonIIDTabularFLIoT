...
class FedGKD(FedAvg):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[BaseException]) \
        ...
        loss, accuracy, ce_loss_result, metrics_dict = evaluate_nn_model(x_test=x_test, y_test=y_test,
                                                                         model=self._model,
        ...
        # self.global_ensemble.append(current_fed_avg_srv_weights)

        # Flower want parameters -> Give it the latest global models params
