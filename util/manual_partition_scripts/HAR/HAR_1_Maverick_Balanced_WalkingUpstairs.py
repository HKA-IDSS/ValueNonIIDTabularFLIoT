...
if __name__ == "__main__":
    ...
    partition_name = "HAR_1_Maverick_1_Balanced_WalkingUpstairs_5"
    ...
    label_distribution_client_train = [list(map(lambda x: int(x / 6), total_label_distribution_train[:-1]))
                                       for _ in clients]
    label_distribution_client_train[-1].append(int(total_label_distribution_train[-1] / 6))
    ...
    label_distribution_client_test = [list(map(lambda x: int(x / 6), total_label_distribution_test[:-1]))
                                      for _ in clients]
    label_distribution_client_test[-1].append(int(total_label_distribution_test[-1] / 6))
    ...
    label_distribution_client_train[-1] = np.subtract(label_distribution_client_train[-1],
                                                      np.array(math.trunc(total_samples_train_non_mavericks / 36),
                                                               ndmin=len(label_distribution_client_train[0]))).reshape(-1)
