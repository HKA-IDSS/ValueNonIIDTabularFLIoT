...
if __name__ == "__main__":
    ...
    label_distribution_client_test = [list(map(lambda x: int(x / 6), total_label_distribution_test[1:]))
                                      for _ in clients]
    label_distribution_client_train[0].insert(0, math.trunc(total_label_distribution_train[0] / 6))
    label_distribution_client_test[0].insert(0, math.trunc(total_label_distribution_test[0] / 6))
    ...
    label_distribution_client_train[0] = np.subtract(label_distribution_client_train[0],
                                                     np.array(math.trunc(total_samples_train_non_mavericks / 36),
                                                              ndmin=len(label_distribution_client_train[0]))).reshape(
        -1)
    label_distribution_client_test[0] = np.subtract(label_distribution_client_test[0],
                                                    np.array(math.trunc(total_samples_test_non_mavericks / 36),
