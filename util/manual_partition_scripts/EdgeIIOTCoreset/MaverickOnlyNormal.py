...
if __name__ == "__main__":
    ...
    partition_name = "EdgeIIOT_Maverick_OnlyNormal"

    labels = y_train.columns
    total_label_distribution_train = [len(y_train[y_train[label] == 1.0]) for label in labels]
    label_distribution_client_train = [list(map(lambda x: int(x / 4), total_label_distribution_train[:7]))
                                       for _ in clients]
    #
    total_label_distribution_test = [len(y_test[y_test[label] == 1.0]) for label in labels]
    label_distribution_client_test = [list(map(lambda x: int(x / 4), total_label_distribution_test[:7]))
                                      for _ in clients]

    for client_number in range(3):
        label_distribution_client_train[client_number].insert(7, 0)
        label_distribution_client_test[client_number].insert(7, 0)
        label_distribution_client_train[client_number] += list(map(lambda x: int(x / 4),
                                                                   total_label_distribution_train[8:]))
        label_distribution_client_test[client_number] += list(map(lambda x: int(x / 4),
                                                                  total_label_distribution_test[8:]))

    label_distribution_client_train[-1].insert(7, total_label_distribution_train[7])
    label_distribution_client_test[-1].insert(7, total_label_distribution_test[7])
    label_distribution_client_train[-1] += list(map(lambda x: int(x / 4), total_label_distribution_train[8:]))
    label_distribution_client_test[-1] += list(map(lambda x: int(x / 4), total_label_distribution_test[8:]))
